...
const SettingItem = ({ icon, label, value, hasToggle, onPress }: SettingItemProps) => {
    const [isEnabled, setIsEnabled] = React.useState(true)

    return (
        <Pressable style={({ pressed }) => [styles.settingItem, pressed && styles.settingItemPressed]} onPress={onPress}>
            ...
            {hasToggle ? (
                <Switch value={isEnabled} onValueChange={setIsEnabled} trackColor={{ false: theme.colors.backgroundSecondary, true: theme.colors.primary }} thumbColor={theme.colors.text} />
            ) : (
...
const BlockedAppSettingsScreen = () => {
        ...
        <View style={[styles.container, { paddingTop: insets.top }]}>
            ...
            <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.section}>
                    ...
                    <SettingItem icon={<Clock size={20} color={theme.colors.primary} />} label='Total Lock Time' value='14h 30m' />
                    <SettingItem icon={<History size={20} color={theme.colors.primary} />} label='Last Lock' value='2h ago' />
                </View>

                <View style={styles.section}>
                    <Text style={styles.sectionTitle}>Schedule</Text>
                    <SettingItem icon={<Calendar size={20} color={theme.colors.primary} />} label='Lock Schedule' value='Daily' />
                </View>

                <View style={styles.section}>
                    <Text style={styles.sectionTitle}>Lock Settings</Text>
                    <SettingItem icon={<Bell size={20} color={theme.colors.primary} />} label='Lock Notifications' hasToggle />
                    <SettingItem icon={<Lock size={20} color={theme.colors.primary} />} label='Block App Launch' hasToggle />
                    <SettingItem icon={<Settings2 size={20} color={theme.colors.primary} />} label='Block Built-in Functions' hasToggle />
                </View>
            ...
            <View style={[styles.footer, { paddingBottom: insets.bottom + 16 }]}>
                <Pressable style={({ pressed }) => [styles.removeButton, pressed && styles.buttonPressed]}>
...
export default BlockedAppSettingsScreen
