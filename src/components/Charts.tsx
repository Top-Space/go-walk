        ...
        >
            <Svg width={width} height={240}>
                ...
                {yAxisLabels.map((label, index) => (
                    <SvgText key={index} x='20' y={30 + index * 45} textAnchor='end' fontSize='12' fill={theme.colors.textSecondary}>
                        {label}
                    </SvgText>
                ))}
                ...
                {yAxisLabels.map((_, index) => (
                    <Line key={index} x1='40' y1={30 + index * 45} x2={width} y2={30 + index * 45} stroke={theme.colors.border} strokeWidth='1' opacity={0.2} />
                ))}
                ...
                {data.map((item, index) => {
                    ...
                    return (
                        <View key={index}>
                            <AnimatedRect x={x} width={barWidth} fill={theme.colors.primary} rx={8} animatedProps={animatedProps} />
                            <SvgText x={x + barWidth / 2} y={230} textAnchor='middle' fontSize='12' fill={theme.colors.textSecondary}>
                                {item.hour}
                            </SvgText>
                        </View>
                    )
    ...
    return (
        ...
        >
            <Svg width={width} height={220}>
                ...
                {/* Line */}
                <AnimatedPath d={pathString} fill='none' stroke={theme.colors.secondary} strokeWidth='3' strokeLinecap='round' strokeLinejoin='round' strokeDasharray='1000' animatedProps={animatedProps} />
                ...
                {data.map((item, index) => {
                    ...
                    return (
                        <SvgText key={index} x={x} y={215} textAnchor='middle' fontSize='12' fill={theme.colors.textSecondary}>
                            {item.day}
                        </SvgText>
                    )
                })}
